...
This project provides one local application for loading a CS2 demo, running player-vision, exposure-gap, or flash-coverage simulations, replaying them over time, and inspecting the result in 3D. It also includes a grenade-lineup extractor that produces stand/aim references and CS2 console commands. Analysis results stay in memory; a `.glb` is generated only when you explicitly export a snapshot.
...
2. In Vision, select one or more players, choose the shared instant or interval on their alive-period round clock, and enable vision coloring, gap coloring, or both.
3. In Flash, select a recorded flash or switch to **Placed flash** and enable **Move flash**.
4. Click the task-specific **Analyze vision / gaps** or **Analyze flash** button and inspect the result immediately.

The **Lineups** tab needs no Analyze button: grenade throws are detected while the demo loads. Filter them by round, grenade, player, or fixed/in-motion reference; select one to see the approach, release, recorded detonation, and collision-limited aim rays. Fixed throws retain their blue reference/orange release markers and one yellow reference ray; in-motion throws use purple for pin pull and blue for actual release, with a matching collision-limited look ray for both moments, while detonation is red. Event times use the same reverse-counting round clock as Vision. Aim distance and the lineup-camera FOV are adjustable, and **View aim** enters the recorded player viewpoint with a centered crosshair. **Copy commands** copies a one-line `setpos`/`setang` sequence, and the filtered result can be downloaded as JSON or CFG. Lineups are preserved in schema-v3 `.cs2session` archives.

The parsed demo, map geometry, interior face samples, and CPU/GPU raycaster remain available for repeated analyses. Identical requests reuse their existing result. With NVIDIA Warp, static samples stay in GPU memory while distance/FOV filtering, raycasting, cumulative sample tracking, face reduction, and bit packing run in fused CUDA kernels; only compact masks or intensities return to the CPU. This optimization does not reduce the configured sample count, pose cadence, range, or visibility threshold.

The Vision tab has a team-grouped multi-player selector. All selected players share one time window, capped at the earliest selected death, and their paths and endpoint poses preview together. Vision coloring is green and remains cumulative in **Accumulated** playback mode. Gap coloring is bright translucent red and marks static-map surfaces that have unobstructed line of sight to the player while remaining outside that player's configured FOV; it is always computed for the current replay tick and never accumulates. Each player receives a distinct hue within the green/red family. Cyan indicates overlapping vision from multiple players, magenta indicates overlapping exposure gaps, and yellow indicates a face carrying both kinds of coverage. Generation and playback each provide independent Vision and Gaps toggles.

Instant mode displays one low-poly aiming pose per selected player; interval mode displays every selected player's start and end pose, including recorded yaw, pitch, and crouch state. These selection poses disappear when an analysis opens, and all selected player models follow the replay. Flash analysis supports demo events and an interactive placed flash; event lists use the same round clock and no coordinate entry is required. Player aiming, crouching, grenade holding/throwing, and flash markers use tiny generated GLB assets instead of generic octahedrons. A placed flash can copy any recorded pop position from the team-grouped selector, then **Move flash** keeps it centered in view and moves it relative to the camera with WASD, Q/E for height, and Shift for faster movement. The viewport shows throttled one-sample coverage previews while moving and after stopping. These previews are transient and do not fill Analysis history; the configured full-quality calculation runs only when **Analyze flash** is clicked. Selecting a recorded flash can automatically focus it, and Flash camera places the viewpoint at its exact position for unobstructed look-around inspection while hiding all overlay objects and lines.
...
**Save session** writes every non-discarded Analysis history result into a self-contained `.cs2session` automatically. Schema-v3 archives contain normalized poses, player life windows, round clocks, events, geometry, and all active compact results; they reopen without the original demo and can run new analyses using the saved fields. Older session schemas are intentionally rejected. Use **Export GLB** only when you want a displayed result or history entry as a portable snapshot.
...
```powershell
# Require two different interior samples to be visible before a face is green.
...
Relative exports are written to `out/`. Each JSON record contains the exact release pose, detected pin-pull pose, recorded detonation position when the event type exposes one, an optional fixed reference pose, a generated movement instruction, a movement path for in-motion fallbacks, separate click/movement/jumpthrow fields, velocity, notes, and ready-to-paste `setpos`/`setang` commands. Use `--grenade-type flashbang` (repeatable) to filter the result.
