...
def main():
    ...
    # Build all rays for all sampled ticks up front, then fire them in chunks.
    # intersects_first only returns which triangle each ray hit (no hit-point
    # math), and batching many ticks per call cuts per-call overhead a lot
    # compared to one call per tick.
    TICKS_PER_CHUNK = 20  # tune down if RAM is still tight, up if you have headroom

    tick_rows = list(rows.itertuples(index=False))
    for chunk_start in range(0, total_ticks, TICKS_PER_CHUNK):
        chunk = tick_rows[chunk_start:chunk_start + TICKS_PER_CHUNK]

        all_origins = []
        all_directions = []
        for row in chunk:
            eye_pos = np.array([row.X, row.Y, row.Z + EYE_HEIGHT_OFFSET])
            origins, directions = build_view_rays(eye_pos, row.yaw, row.pitch, fov=args.fov, grid=args.grid)
        ...
        # intersects_first returns one triangle index per ray (-1 if no hit)
        hit_tri_ids = mesh.ray.intersects_first(all_origins, all_directions)
        hits = hit_tri_ids[hit_tri_ids != -1]
        seen_triangle_ids.update(hits.tolist())

        processed = min(chunk_start + TICKS_PER_CHUNK, total_ticks)
        print(f"  processed {processed}/{total_ticks} sampled ticks, {len(seen_triangle_ids)} triangles seen so far")
